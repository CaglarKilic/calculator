  ...
  if (digitCounts.all <= MAX_DIGIT && !input.includes("e")) {
    return (
      number.toLocaleString("en-US", {
        minimumFractionDigits: digitCounts.decimal,
      }) + endDot
    );
  }

  if (fixed.match(/[^0\.]+$/) && !fixed.includes("e")) {
    return fixed;
  }
...
function registerBuffers(op) {
  ...
  if (numBuffer.length > opBuffer.length) {
    opBuffer.push(op);
  } else {
    opBuffer.pop();
    opBuffer.push(op);
...
function reduce(mode = "r", startIndex = 0) {
  const lvalue =
    mode == "w" ? numBuffer.splice(startIndex, 1)[0] : numBuffer[startIndex];
  const rvalue =
    mode == "w"
      ? numBuffer.splice(startIndex, 1)[0]
      : numBuffer[startIndex + 1];
  const op =
    mode == "w" ? opBuffer.splice(startIndex, 1)[0] : opBuffer[startIndex];

  const result = arith[op](lvalue, rvalue);

  if (mode == "w") {
    startIndex ? numBuffer.push(result) : numBuffer.unshift(result);
  }
...
function addDigit() {
  let input = read();
  if (countDigits(input).all == MAX_DIGIT) {
    return;
  }
  if (mem) {
    flush("all");
...
function addDecimal() {
  let input = read() || "0";
  if (countDigits(input).all == MAX_DIGIT || input.includes(".")) {
    return;
  }
  buffer = input + this.textContent;
  write(format(buffer));
...
function operate() {
  ...
  if (opBuffer.length == 1 && numBuffer.length == 2) {
    write(reduce("w"));
  } // only helps function `equate`

  if (numBuffer.length == 2) {
    if (checkPrecedence(opBuffer[0])) {
      write(reduce("w"));
    } else if (checkPrecedence(opBuffer[1])) {
      write(numBuffer[1]);
    } else {
      write(reduce());
    }
